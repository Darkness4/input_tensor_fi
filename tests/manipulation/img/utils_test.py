...
def test_perturb_image_tensor():
    pixels = np.array(
        [PixelFault(16, 16, 255, 255, 0), PixelFault(5, 5, 255, 255, 0)],
        dtype=object,
    )
    (_, _), (x_test, _) = cifar10.load_data()
    image = tf.constant(x_test[0])
    perturb_image = utils.build_perturb_image_tensor(pixels)

    images_pertubed = perturb_image(image).numpy()
...
def test_perturb_image_by_bit_fault_tensor():
    bit_faults = np.array(
        [BitFault(16, 16, 0, 3, BitFlip)],
        dtype=object,
    )
    (_, _), (x_test, _) = cifar10.load_data()
    image = tf.constant(x_test[0])
    perturb_image = utils.build_perturb_image_by_bit_fault_tensor(bit_faults)

    images_pertubed = perturb_image(image).numpy()

    assert image[16, 16, 0].numpy() != images_pertubed[16, 16, 0]
